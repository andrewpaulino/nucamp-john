...
class CommentSection extends React.Component {
    ...
    addNewComment = () => {
        ...
        this.setState({comments: [...this.state.comments, {name: this.state.nameValue, message: this.state.commentValue}] });
    }
    ...
    render() {
        ...
        const renderComments = this.state.comments.map(comment => {
            return (
                <CommentModule name={comment.name} message={comment.message} />
            );
        })
        return( 
        <>
        ...
        <div>
            Message: 
            <input name="commentValue" type='text' onChange={(e) => this.onChangeValue(e)} value={this.state.commentValue} id='messageinput'/>
            Name: 
            <input name="nameValue" type='text' onChange={(e) => this.onChangeValue(e)} value={this.state.nameValue} id='nameinput'/>
            <button id='inputButton' onClick={()=> this.addNewComment()}>Add Comment</button>
        </div>
        </>);
    }
}
